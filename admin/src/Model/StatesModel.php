...
class StatesModel extends ListModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_businessdirectory/assets/css/states.css'
 	];
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('state.access', 'com_businessdirectory.state.' . (int) $item->id) && $user->authorise('state.access', 'com_businessdirectory'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__businessdirectory_state', 'a'));
