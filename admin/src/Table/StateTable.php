...
class StateTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_businessdirectory.state';
		...
		parent::__construct('#__businessdirectory_state', 'id', $db, $dispatcher);
	}
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_BUSINESSDIRECTORY_STATE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_BUSINESSDIRECTORY_STATE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_businessdirectory.state.'.$this->id);
			$this->setRules($rules);
		}
