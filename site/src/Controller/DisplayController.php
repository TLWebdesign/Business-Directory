...
/------------------------------------------------------------------------------------------------------*/
namespace JCB\Component\Businessdirectory\Site\Controller;
...
/**
 * Businessdirectory master site display controller.
 *
 * @since   4.0
 */
class DisplayController extends BaseController
{
	...
	function display($cachable = false, $urlparams = false)
	{
		...
		if ($isEdit && !$this->checkEditId($view, $id))
		{
			...
			if ($refid > 0 && StringHelper::check($ref))
			{
				...
				$this->setRedirect(Route::_('index.php?option=com_businessdirectory&view=' . (string) $ref . '&layout=edit&id=' . (int) $refid, false));
			}
			elseif (StringHelper::check($ref))
			{
				// redirect to ref
				 $this->setRedirect(Route::_('index.php?option=com_businessdirectory&view=' . (string) $ref, false));
			}
			else
			{
				// normal redirect back to the list default site view
				$this->setRedirect(Route::_('index.php?option=com_businessdirectory&view=', false));
			}
	...
	{
		if (parent::checkEditId("com_businessdirectory.edit.{$context}", $id))
		{
			return true;
		}
		...
		{
			$this->holdEditId("com_businessdirectory.edit.{$context}", $id);
	...
	protected function canEditId($view, $id): bool
	{
		...
		{
			$access = ($this->identity->authorise($allowed['access'], "com_businessdirectory.{$view}." . (int) $id)
				&& $this->identity->authorise($allowed['access'], 'com_businessdirectory'));
		}
		...
		{
			$edit = ($this->identity->authorise($allowed['edit'], "com_businessdirectory.{$view}." . (int) $id)
				&& $this->identity->authorise($allowed['edit'], 'com_businessdirectory'));
		}

		// check if this item can be edited by owner
		if ($access && !$edit && isset($allowed['edit.own']))
		{
			$edit = ($this->identity->authorise($allowed['edit.own'], "com_businessdirectory.{$view}." . (int) $id)
				&& $this->identity->authorise($allowed['edit.own'], 'com_businessdirectory'));
		}
