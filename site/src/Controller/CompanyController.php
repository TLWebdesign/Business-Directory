...
/-------------------------------------------------------------------------------------------------------/
	...
	@subpackage		CompanyController.php
...
class CompanyController extends FormController
{
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_BUSINESSDIRECTORY_COMPANY';
	...
	protected function allowAdd($data = [])
	{
		...
		// Access check.
		$access = $user->authorise('company.access', 'com_businessdirectory');
		if (!$access)
		{
			return false;
		}
	...
	protected function allowEdit($data = [], $key = 'id')
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit', 'com_businessdirectory.company.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('core.edit.own', 'com_businessdirectory.company.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('core.edit.own', 'com_businessdirectory'))
						{
							return true;
						}
					}
	...
	public function batch($model = null)
	{
		...
		// Preset the redirect
		$this->setRedirect(Route::_('index.php?option=com_businessdirectory&view=companies' . $this->getRedirectToListAppend(), false));
