...
use JCB\Joomla\Data\Factory as DataFactory;
...
/**
 * Businessdirectory Country Admin Model
 *
 * @since  1.6
 */
class CountryModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_businessdirectory/assets/css/country.css'
 	];
	...
	protected array $scripts = [
		...
		'media/com_businessdirectory/js/country.js'
 	];
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_businessdirectory.country';
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			$item->states = DataFactory::_('Data.Subform')->table('state')->get($item->guid ?? '' ,'country', 'states', ['guid','name']);
		}
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_businessdirectory.country', 'country', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('country.edit.state', 'com_businessdirectory.country.' . (int) $id))
			|| ($id == 0 && !$user->authorise('country.edit.state', 'com_businessdirectory')))
		{
		...
		if (!$user->authorise('core.edit.created_by', 'com_businessdirectory'))
		{
			// Disable fields for display.
		...
		if (!$user->authorise('core.edit.created', 'com_businessdirectory'))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('country.edit.name', 'com_businessdirectory.country.' . (int) $id))
			|| ($id == 0 && !$user->authorise('country.edit.name', 'com_businessdirectory')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('country.access.name', 'com_businessdirectory.country.' . (int) $id))
			|| ($id == 0 && !$user->authorise('country.access.name', 'com_businessdirectory')))
		{
			// Remove the field
			$form->removeField('name');
		}
		// Modify the form based on View Name access controls.
		if ($id != 0 && (!$user->authorise('country.view.name', 'com_businessdirectory.country.' . (int) $id))
			|| ($id == 0 && !$user->authorise('country.view.name', 'com_businessdirectory')))
		{
		...
		if ($id != 0 && (!$user->authorise('country.edit.states', 'com_businessdirectory.country.' . (int) $id))
			|| ($id == 0 && !$user->authorise('country.edit.states', 'com_businessdirectory')))
		{
			// Disable fields for display.
			...
			{
				// Disable fields while saving.
				$form->setFieldAttribute('states', 'filter', 'unset');
				// Disable fields while saving.
				$form->setFieldAttribute('states', 'required', 'false');
			}
		}
...
/***[/INSERTED$$$$]***/
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('country.delete', 'com_businessdirectory.country.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('country.edit.state', 'com_businessdirectory.country.' . (int) $recordId);
		...
		return $user->authorise('country.edit.state', 'com_businessdirectory');
	}
	...
	protected function allowEdit($data = [], $key = 'id')
	{
		...
		return $user->authorise('country.edit', 'com_businessdirectory.country.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('country.edit',  'com_businessdirectory');
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__businessdirectory_country'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_businessdirectory.edit.country.data', []);
		...
		$this->preprocessData('com_businessdirectory.country', $data);
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo = BusinessdirectoryHelper::getActions('country');
		$this->batchSet = true;
...
/***[JCBGUI.admin_view.php_before_save.303.$$$$]***/
