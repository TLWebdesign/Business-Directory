...
class CountriesModel extends ListModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_businessdirectory/assets/css/countries.css'
 	];
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('country.access', 'com_businessdirectory.country.' . (int) $item->id) && $user->authorise('country.access', 'com_businessdirectory'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__businessdirectory_country', 'a'));
	...
	protected function checkInNow(): bool
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__businessdirectory_country'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__businessdirectory_country'))->set($fields)->where($conditions); 
