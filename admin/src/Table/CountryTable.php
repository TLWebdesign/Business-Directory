...
class CountryTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_businessdirectory.country';
		...
		parent::__construct('#__businessdirectory_country', 'id', $db, $dispatcher);
	}
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_BUSINESSDIRECTORY_COUNTRY_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_BUSINESSDIRECTORY_COUNTRY_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_businessdirectory.country.'.$this->id);
			$this->setRules($rules);
		}
